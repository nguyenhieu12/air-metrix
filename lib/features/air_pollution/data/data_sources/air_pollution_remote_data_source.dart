...
abstract class AirPollutionRemoteDataSource {
  ...
  Future<AirPollutionModel> getCompareAirPollution(double lat, double long);
...
class AirPollutionRemoteDataSourceImpl implements AirPollutionRemoteDataSource {
  ...
  Future<AirPollutionModel> getCurrentAirPollution(
    ...
    if (respone.statusCode == 200 && next3Days.statusCode == 200) {
      ...
      cubit.list12HoursForecastAQI.clear();
      cubit.list6HoursForecastAQI.clear();

      for (int i = 0; i < 7; i++) {
        cubit.list6HoursForecastAQI.add(next3DaysForecast[i]['main']['aqi']);
      }

      for (int i = 0; i < 13; i += 2) {
        cubit.list12HoursForecastAQI.add(next3DaysForecast[i]['main']['aqi']);
      }

      for (int i = 0; i < next3DaysForecast.length; i += 24) {
        cubit.list3DaysForecastAQI.add(next3DaysForecast[i]['main']['aqi']);
      }
  ...
  @override
  Future<AirPollutionModel> getCompareAirPollution(
      double lat, double long) async {
    DateTime now = DateTime.now();
    ...
    if (respone.statusCode == 200 && next3Days.statusCode == 200) {
      AirCompareCubit cubit = Injector.instance();
